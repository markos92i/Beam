...
public struct RequestBuilderState {
    ...
    var config: RequestConfig = .standard
...
public struct RequestBuilder {
    ...
    public static func buildFinalResult<Success, Failure>(_ components: [RequestComponent]) -> StreamTask<Success, Failure> {
        StreamTask(service: buildService(from: components))
    }

    public static func buildFinalResult(_ components: [RequestComponent]) -> [RequestComponent] { components }

    private static func buildState(from components: [RequestComponent]) -> RequestBuilderState {
    ...
    private static func buildService<Success, Failure>(from components: [RequestComponent]) -> Service<Success, Failure> {
        buildService(from: buildState(from: components))
    }

    private static func buildService<Success, Failure>(from state: RequestBuilderState) -> Service<Success, Failure> {
        let api = APIRequest(
            method: state.method,
            host: state.host,
            path: state.path,
